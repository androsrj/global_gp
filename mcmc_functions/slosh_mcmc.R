...
mcmc <- function(X, Z, Y, D, K,
  ...
  A2 <<- A2
  A2Test <- matrix(0, nTest, nTest * q)
  for (j in seq_len(nTest)) {
    cols <- ((j - 1) * q + 1):(j * q)
    A2Test[j, cols] <- ATest[j, ]
  }
  A2Test <<- A2Test
  ...
  little.b <- Reduce("+", lapply(1:S, function(s) {
    ind <- ((s-1)*n+1):(s*n)
    Y2 <- Y[ind, ]
    crossprod(A2 %*% P, Y2) / starting$tau2
  }))
  mu <- big.B %*% little.b
  ...
  big.B <- solve(STest * crossprod(A2Test %*% PTest) / starting$tau2 + Sigma.inv)
  little.b <- Reduce("+", lapply(1:STest, function(s) {
    ind <- ((s-1)*nTest+1):(s*nTest)
    Y2Test <- YTest[ind, ]
    crossprod(A2Test %*% PTest, Y2Test) / starting$tau2
  }))
  ...
  XBTest <<- rep(1, STest) %x% rowSums(ATest * current.beta.test)
  ...
  for (i in 2:nIter) {
    ...
    Sigma.inv <- as.matrix(bdiag(Sigma.q.inv))
    big.B <- solve(S * crossprod(A2 %*% P) / tau2 + Sigma.inv)
    
    # Little b
    little.b <- Reduce("+", lapply(1:S, function(s) {
      ind <- ((s-1)*n+1):(s*n)
      Y2 <- Y[ind, ]
      crossprod(A2 %*% P, Y2) / tau2
    }))
    ...
    Sigma.inv <- as.matrix(bdiag(Sigma.q.inv))
    big.B <- solve(STest * crossprod(A2Test %*% PTest) / tau2 + Sigma.inv)
    
    # Little b
    little.b <- Reduce("+", lapply(1:STest, function(s) {
      ind <- ((s-1)*nTest+1):(s*nTest)
      Y2Test <- YTest[ind, ]
      crossprod(A2Test %*% PTest, Y2Test) / tau2
    }))
    ...
    DBTest <- lapply(1:q, \(j) matrix(ATest[ , j], nrow = nTest, ncol = nTest) *
                       ...
                       matrix(ATest[ , j], nrow = nTest, ncol = nTest, byrow = T))
