...
mcmc <- function(X, Z, Y, D, K,
  ...
  DB <- lapply(1:q, \(j) matrix(X0[ , j], nrow = n, ncol = n) *
                 (starting$sigb2[j] * 
                    (1 + sqrt(3) * starting$thb[j] * D) * 
                    exp(-sqrt(3) * starting$thb[j] * D)) *
  ...
  DBTest <- lapply(1:q, \(j) matrix(X0Test[ , j], nrow = nTest, ncol = nTest) *
                     (starting$sigb2[j] * 
                        (1 + sqrt(3) * starting$thb[j] * DTest) * 
                        exp(-sqrt(3) * starting$thb[j] * DTest)) *
  ...
  Sigma.q.inv <- lapply(1:q, \(j) solve(starting$sigb2[j] * 
                                          (1 + sqrt(3) * starting$thb[j] * D) * 
                                          exp(-starting$thb[j] * D)))
  ...
  for (i in 2:nIter) {
    ...
    # Big B
    Sigma.q.inv <- lapply(1:q, \(j) solve(sigb2[j] * 
                                            (1 + sqrt(3) * thb[j] * D) * 
                                            exp(-sqrt(3) * thb[j] * D)))
    Sigma.inv <- as.matrix(bdiag(Sigma.q.inv))
    ...
    # Big B
    Sigma.q.inv <- lapply(1:q, \(j) solve(sigb2[j] * 
                                            (1 + sqrt(3) * thb[j] * DTest) * 
                                            exp(-sqrt(3) * thb[j] * DTest)))
    Sigma.inv <- as.matrix(bdiag(Sigma.q.inv))
    ...
    DBTest <- lapply(1:q, \(j) matrix(X0Test[ , j], nrow = nTest, ncol = nTest) *
                       (sigb2[j] * (1 + sqrt(3) * thb[j] * DTest) * exp(-sqrt(3) * thb[j] * DTest)) *
                       matrix(X0Test[ , j], nrow = nTest, ncol = nTest, byrow = T))
