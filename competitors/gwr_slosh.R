...
for (j in 1:nSubj) {
  ...
  if (j == 1) {
    saveRDS(preds.test, file = "gwr_preds_subj1.RDS")
  }
...
results <- c(std.dev = round(sd(test$Y), 2), 
             rmse = round(mean(rmse), 2), 
             cvg = round(mean(cvg), 2), 
             len = round(mean(len), 2))
results
